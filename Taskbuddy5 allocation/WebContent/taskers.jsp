...
<a style="color:white; font-weight:bolder; " href="menu2?type=${param.type}&category=${catlist}" name="${cat}">${catlist}<br/></a>
</c:forEach> 
</div>
        </div> --%>
        ...
        <div class="col-lg-4">
          ...
          <h2>${item }</h2>
          <p>Experienced Tasker and professional Skill services</p>
          <p><a class="btn btn-default" href="categorylist?location=${item}" name="${item}">${item}<br/> &raquo;</a></p>
         ...
         <div class="row">
          <div class="col-lg-4">
          <h2><a href="bookTasker?tasker=${item.taskerid}">${item.userFirstName} ${item.userLastName}</a></h2>
          <p>contact no: ${item.contactno}</p>
          <p>charge: ${item.charge}</p>
          <p>rating: ${item.rating}</p>
          <p><button class="button" type="button" onclick="window.location.href='displayReviews?tasker=${item.taskerid}'">show reviews</button></p>
          </div>
